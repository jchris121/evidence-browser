...
def scan_discoveries_from_db(conn, device_map):
    ...
    for device_id, info in device_map.items():
        ...
        for term in high_terms_3:
            rows = conn.execute(
                "SELECT data, timestamp, category FROM records WHERE device_id=? AND category IN ('chats','emails') AND searchable LIKE ? LIMIT 50",
                (device_id, f"%{term}%")
            ).fetchall()
            seen_dates = set()
            for r in rows:
        ...
        for term in high_terms_2:
            rows = conn.execute(
                "SELECT data, timestamp, category FROM records WHERE device_id=? AND category IN ('chats','emails') AND searchable LIKE ? LIMIT 20",
                (device_id, f"%{term}%")
            ).fetchall()
            seen_dates = set()
            for r in rows:
        ...
        for date_str, (label, flames) in critical_dates_list:
            rows = conn.execute(
                "SELECT data, category FROM records WHERE device_id=? AND category='chats' AND timestamp LIKE ? LIMIT 5",
        ...
        for date_str, (label, flames) in critical_dates_list:
            count = conn.execute(
                "SELECT COUNT(*) FROM records WHERE device_id=? AND category='calls' AND timestamp LIKE ?",
                (device_id, f"{date_str}%")
            ).fetchone()[0]
            if count:
                disc_id += 1
        ...
        for date_str, (label, flames) in critical_dates_list:
            rows = conn.execute(
                "SELECT data FROM records WHERE device_id=? AND category='locations' AND timestamp LIKE ? LIMIT 3",
                (device_id, f"{date_str}%")
            ).fetchall()
            if rows:
        ...
        # --- Suspicious searches ---
        rows = conn.execute(
            "SELECT data, timestamp FROM records WHERE device_id=? AND category='searches'", (device_id,)
        ).fetchall()
        for r in rows:
            rec = json.loads(r["data"])
            query = rec.get("query", "")
        ...
        # --- Passwords ---
        pwd_count = conn.execute(
            "SELECT COUNT(*) FROM records WHERE device_id=? AND category='passwords'", (device_id,)
        ).fetchone()[0]
        if pwd_count:
            disc_id += 1
            discoveries.append({
    ...
    rows = conn.execute("SELECT device_id, data FROM records WHERE category='contacts'").fetchall()
    for r in rows:
        rec = json.loads(r["data"])

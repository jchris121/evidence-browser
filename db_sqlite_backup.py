"""SQLite-backed evidence index with FTS5 for instant search.
Parses markdown once → stores in SQLite → serves from memory/DB.
"""
import sqlite3
import os
import re
...
class EvidenceDB:
    ...
    def init_schema(self):
        ...
        stmts = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_device_cat ON records(device_id, category)",
            "CREATE INDEX IF NOT EXISTS idx_timestamp ON records(timestamp)",
            """CREATE TABLE IF NOT EXISTS devices (
                device_id TEXT PRIMARY KEY,
                name TEXT, type TEXT, owner TEXT, source TEXT
            )""",
            """CREATE TABLE IF NOT EXISTS device_category_counts (
                device_id TEXT, category TEXT, count INTEGER,
                PRIMARY KEY (device_id, category)
            )""",
            """CREATE TABLE IF NOT EXISTS file_index (
                file_path TEXT PRIMARY KEY, mtime REAL, record_count INTEGER
            )""",
            """CREATE TABLE IF NOT EXISTS discoveries (
                id TEXT PRIMARY KEY, title TEXT, category TEXT, flames INTEGER,
                device_id TEXT, owner TEXT, content TEXT, timestamp TEXT,
                verified INTEGER DEFAULT 0, tags TEXT, data_type TEXT, source_app TEXT
            )""",
            "CREATE INDEX IF NOT EXISTS idx_disc_flames ON discoveries(flames DESC)",
            "CREATE INDEX IF NOT EXISTS idx_disc_cat ON discoveries(category)",
            ...
            "CREATE INDEX IF NOT EXISTS idx_threads_device ON chat_threads(device_id)",
            ...
            "CREATE INDEX IF NOT EXISTS idx_chatmsg_thread ON chat_messages(device_id, thread_num)",
        ]
        for stmt in stmts:
            c.execute(stmt)
        # FTS5
        try:
            c.execute("CREATE VIRTUAL TABLE IF NOT EXISTS records_fts USING fts5(searchable, content=records, content_rowid=id)")
        except Exception:
            pass
        c.commit()

    def full_index(self, force=False):
        """Parse all data sources and load into SQLite.
        ...
        try:
            ...
            if count > 0 and not force:
                ...
                self._last_index_time = os.path.getmtime(str(DB_PATH))
                self._rag_chunk_count = self._get_rag_count()
                self._rebuild_caches()
                # Recompute discoveries if empty
                disc_count = c.execute("SELECT COUNT(*) FROM discoveries").fetchone()[0]
                if disc_count == 0:
                    self._compute_discoveries(c)
                return
        except Exception:
            pass
        ...
        c.execute("DELETE FROM device_category_counts")
        c.execute("DELETE FROM file_index")
        c.execute("DELETE FROM chat_threads")
        c.execute("DELETE FROM chat_messages")

        # Index Cellebrite
        self._index_cellebrite(c)
        # Index AXIOM metadata
        self._index_axiom_metadata(c)
        # Build FTS
        ...
        for person_id, info in DEVICE_MAP.items():
            ...
            for cat in CATEGORIES:
                ...
                for rec in parser(fpath):
                    ...
                    count += 1
                    if len(batch) >= 5000:
                        c.executemany("INSERT INTO records (device_id,category,timestamp,data,searchable) VALUES (?,?,?,?,?)", batch)
                        batch = []
                if batch:
                    c.executemany("INSERT INTO records (device_id,category,timestamp,data,searchable) VALUES (?,?,?,?,?)", batch)
                c.execute("INSERT OR REPLACE INTO device_category_counts VALUES (?,?,?)", (person_id, cat, count))
                c.execute("INSERT OR REPLACE INTO file_index VALUES (?,?,?)", (str(fpath), fpath.stat().st_mtime, count))
                if count:
                    _log.debug(f'{person_id}/{cat}: {count}')
                # Parse chat threads
                if cat == 'chats':
                    self._index_chat_threads(c, person_id, fpath)
        c.commit()

    def _index_axiom_metadata(self, c):
        ...
        for dirname in sorted(os.listdir(AXIOM_DIR)):
            ...
            for jf in dirpath.glob("*.json"):
                ...
                est = max(1, fsize // 500)
                c.execute("INSERT OR REPLACE INTO device_category_counts VALUES (?,?,?)", (dirname, jf.stem, est))
                total += est
            c.execute("INSERT OR REPLACE INTO file_index VALUES (?,?,?)",
                      (str(dirpath), dirpath.stat().st_mtime, total))
        c.commit()

    def _index_chat_threads(self, c, device_id, fpath):
        ...
        if thread_batch:
            c.executemany("INSERT INTO chat_threads (device_id,thread_num,source_app,started,first_date,last_date,message_count,participants,last_message_preview) VALUES (?,?,?,?,?,?,?,?,?)", thread_batch)
        if msg_batch:
            c.executemany("INSERT INTO chat_messages (device_id,thread_num,timestamp,sender,body,source_app) VALUES (?,?,?,?,?,?)", msg_batch)
    ...
    def get_chat_threads(self, device_id, page=1, per_page=50, search=None, date_from=None, date_to=None):
        ...
        params = list(sub_ids)
        if date_from:
            where.append("last_date >= ?")
            params.append(date_from)
        if date_to:
            where.append("first_date <= ?")
            params.append(date_to + 'T23:59:59')
        if search:
            where.append("(participants LIKE ? OR last_message_preview LIKE ? OR source_app LIKE ?)")
            sl = f"%{search}%"
            params.extend([sl, sl, sl])
        where_sql = " AND ".join(where)
        total = c.execute(f"SELECT COUNT(*) FROM chat_threads WHERE {where_sql}", params).fetchone()[0]
        offset = (page - 1) * per_page
        rows = c.execute(f"SELECT * FROM chat_threads WHERE {where_sql} ORDER BY last_date DESC LIMIT ? OFFSET ?",
    ...
    def get_thread_messages(self, device_id, thread_id):
        ...
        sub_ids = self._resolve_device_ids(device_id)
        placeholders = ",".join("?" * len(sub_ids))
        rows = c.execute(f"SELECT * FROM chat_messages WHERE device_id IN ({placeholders}) AND thread_num=? ORDER BY timestamp",
                         sub_ids + [thread_id]).fetchall()
    ...
    def _rebuild_caches(self):
        ...
        rows = c.execute("SELECT * FROM devices ORDER BY source, owner, device_id").fetchall()
        raw_devices = []
        for r in rows:
            cats = {}
            for cr in c.execute("SELECT category, count FROM device_category_counts WHERE device_id=?", (r["device_id"],)):
    ...
    def _compute_discoveries(self, c):
        from discovery_engine import scan_discoveries_from_db
        c.execute("DELETE FROM discoveries")
        discs = scan_discoveries_from_db(c, DEVICE_MAP)
        batch = []
        for d in discs:
            batch.append((
        ...
        c.executemany("INSERT OR REPLACE INTO discoveries VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", batch)
        c.commit()
    ...
    def get_discoveries(self, category="all", person="all", sort="importance", page=1, per_page=50):
        ...
        if self._discoveries_cache is None:
            rows = c.execute("SELECT category, COUNT(*) as cnt FROM discoveries GROUP BY category").fetchall()
        ...
        if person != "all":
            where.append("(owner LIKE ? OR tags LIKE ?)")
        ...
        total = c.execute(f"SELECT COUNT(*) FROM discoveries{where_sql}", params).fetchone()[0]
        offset = (page - 1) * per_page
        rows = c.execute(
            f"SELECT * FROM discoveries{where_sql} ORDER BY {order} LIMIT ? OFFSET ?",
    ...
    def get_device_data(self, device_id, category=None, page=1, per_page=100, query=None, date_from=None, date_to=None):
        ...
        dev = c.execute("SELECT source FROM devices WHERE device_id=?", (sub_ids[0],)).fetchone()
        if not dev:
            return {"records": [], "total": 0, "page": page, "per_page": per_page}

        if dev["source"] == "axiom":
            if len(sub_ids) > 1:
                return self._get_axiom_data_merged(sub_ids, category, page, per_page, query)
            return self._get_axiom_data(sub_ids[0], category, page, per_page, query)

        # Cellebrite — from SQLite
        placeholders = ",".join("?" * len(sub_ids))
        where = [f"device_id IN ({placeholders})"]
        params = list(sub_ids)
        if category:
        ...
        if query:
            ...
            params.append(self._fts_escape(query))

        where_sql = " AND ".join(where)
        total = c.execute(f"SELECT COUNT(*) FROM records WHERE {where_sql}", params).fetchone()[0]
        offset = (page - 1) * per_page
        rows = c.execute(
            f"SELECT data, category FROM records WHERE {where_sql} ORDER BY timestamp DESC LIMIT ? OFFSET ?",
            params + [per_page, offset]
        ).fetchall()
    ...
    def _fts_escape(query):
        """Escape query for FTS5. Wrap each word in quotes for exact prefix matching."""
        words = query.strip().split()
        if not words:
            return '""'
        # Wrap entire phrase in quotes for phrase match
    ...
    def _get_axiom_data(self, device_id, category, page, per_page, query):
        if not category:
            ...
            c = self._get_conn()
            rows = c.execute("SELECT category, count FROM device_category_counts WHERE device_id=? AND count > 0 ORDER BY category",
                             (device_id,)).fetchall()
    ...
    def _get_axiom_data_merged(self, sub_ids, category, page, per_page, query):
        ...
        if not category:
            ...
            cat_totals = defaultdict(int)
            for sid in sub_ids:
                rows = c.execute("SELECT category, count FROM device_category_counts WHERE device_id=? AND count > 0", (sid,)).fetchall()
                for r in rows:
    ...
    def search_all(self, query, device_filter=None, category_filter=None, page=1, per_page=50):
        """FTS5-powered text search — sub-second for most queries."""
        c = self._get_conn()
        where = ["r.id IN (SELECT rowid FROM records_fts WHERE records_fts MATCH ?)"]
        params = [self._fts_escape(query)]
        ...
        where_sql = " AND ".join(where)
        total = c.execute(f"SELECT COUNT(*) FROM records r WHERE {where_sql}", params).fetchone()[0]
        offset = (page - 1) * per_page
        rows = c.execute(f"""
    ...
    def refresh(self):
        ...
        with self._lock:
            t0 = time.time()
            c = self._get_conn()
            changed = self._detect_changes(c)
            if not changed:
            ...
            for fpath_str in changed:
                ...
                if person_id not in DEVICE_MAP:
                    continue
                # Delete old records
                c.execute("DELETE FROM records WHERE device_id=? AND category=?", (person_id, cat))
                # Re-parse
                parser = PARSERS.get(cat, _parse_generic)
                count = 0
                batch = []
    ...
    def _detect_changes(self, c):
        ...
        for person_id in DEVICE_MAP:
            for cat in CATEGORIES:
                ...
                row = c.execute("SELECT mtime FROM file_index WHERE file_path=?", (str(fpath),)).fetchone()
                if not row or row["mtime"] != current_mtime:
                    changed.append(str(fpath))
        return changed

    def _get_rag_count(self):
        """Get RAG chunk count. Non-blocking — returns 0 if DB locked."""
        try:
            rc = sqlite3.connect(str(RAG_DB), timeout=0.5)
            rc.execute("PRAGMA query_only=ON")
            count = rc.execute("SELECT COUNT(*) FROM documents").fetchone()[0]
            rc.close()
            return count
        except Exception:
            # Try reading cached count file instead
            count_file = Path.home() / "clawd/rag/cache/db_count.txt"
            try:
